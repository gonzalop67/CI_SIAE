...
<html lang="es">
    ...
    <head>
        ...
        <title>SIAE Periodo Lectivo</title>
        ...
        <script type="text/javascript">
            ...
            function eliminar(id_periodo_lectivo) {
                if(confirm("¿Seguro que desea eliminar este periodo lectivo?")){
                    $("#img_loader").show();
                    $.post(
                        "<?php echo site_url('periodo_lectivo/eliminar') ?>",
                        {
                        ...
                        function(respuesta){
                            ...
                            alert(resp['mensaje']);
                            location.href = "<?php echo site_url('periodo_lectivo') ?>"
                        }
                    );
                }
            }
    ...
    <body>
        ...
        <div class="row text-center color1">
            <h3>LISTA DE PERIODOS LECTIVOS EXISTENTES</h3>
        </div>
        ...
        <div id="listado" class="table-responsive text-center">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php 
                    if ($periodos_lectivos) {
                        //impresión de los datos.
                        foreach ($periodos_lectivos->result() as $periodo_lectivo) {
                            $id = $periodo_lectivo->id_periodo_lectivo;
                            $nombre = $periodo_lectivo->pe_anio_inicio . " - " . $periodo_lectivo->pe_anio_fin;
                            echo "<tr><td>$id</td>\n";
                            echo "<td>$nombre</td>\n";
                            echo "<td><a href='periodo_lectivo/ver/$id'>Ver</a></td>\n";
                            echo "<td><a href='#' onclick='eliminar(".$id.")'>Eliminar</a></td></tr>\n";
                        }
                    } else {
                        echo "<td colspan='4' align='center'>No se han definido periodos lectivos todavia...</td>";
                    }
                    ?>
                </tbody>
            </table>
        ...
        <div class="text-center" style="margin-bottom: 8px;">
            <a class="btn btn-default" href="<?php echo site_url('periodo_lectivo/nuevo') ?>" role="button">NUEVO PERIODO LECTIVO</a>
        </div>
