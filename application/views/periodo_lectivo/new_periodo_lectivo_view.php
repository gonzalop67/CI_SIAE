...
<html lang="es">
    ...
    <head>
        ...
        <title>SIAE Nuevo Periodo Lectivo</title>
        ...
        <script type="text/javascript">
            $(document).on("ready", function () {
                ...
                $("#new_periodo_lectivo").submit(function(event){
                    ...
                    if(parseInt(anio_inicio) > parseInt(anio_fin)){
                        $("#mensaje-error").css('color','red');
                        $("#mensaje").html("El año de inicio no puede ser mayor que el final...");
                        $("#anio_inicio").focus();
                    }else if(parseInt(anio_inicio) > parseInt(f.getFullYear())){
                        $("#mensaje-error").css('color','red');
                        $("#mensaje").html("El año de inicio no puede ser mayor que al año actual...");
                        $("#anio_inicio").focus();
                    }else if(parseInt(anio_fin) - parseInt(anio_inicio) > 1){
                        $("#mensaje-error").css('color','red');
                        $("#mensaje").html("La diferencia entre el año inicial y final debe ser un año...");
                        $("#anio_inicio").focus();
                    }else{
                        $("#mensaje").html("");
                        $("#img_loader").show();
                        $.post(
                            "<?php echo site_url('periodo_lectivo/recibirdatos'); ?>",
                            {
    ...
    <body>
        ...
        <div class="row text-center color1">
            <h3>INGRESAR UN NUEVO PERIODO LECTIVO</h3>
        </div>
        ...
        <form id="new_periodo_lectivo" class="form-horizontal form-margin" role="form" method="post" action="">
            <div class="form-group" style="margin-top: 8px">
                <label for="anio_inicio" class="col-sm-2 control-label">Año Inicial:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="anio_inicio" name="anio_inicio" type="text" placeholder="Año de Inicio" value="" required autofocus>
                </div>    
            </div>
            <div class="form-group" style="margin-top: 8px">
                <label for="anio_fin" class="col-sm-2 control-label">Año Final:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="anio_fin" name="anio_fin" type="text" placeholder="Año de Finalización" value="" required>
                </div>    
            </div>
