...
<html lang="es">
    ...
    <head>
        ...
        <title>SIAE Nuevo Usuario</title>
        ...
        <script type="text/javascript">
            $(document).on("ready", function () {
                ...
                //Carga de los perfiles definidos en la base de datos mediante AJAX
                $.ajax({
                    url: "<?php echo site_url('perfil/getPerfiles')?>",
                    success: function(datos){
                        var registros = JSON.parse(datos);
                        $.each(registros,function(i,item){
                           $("#cboPerfil") .append('<option value="'+item.id_perfil+'">'+item.pe_nombre+'</option>');
                        });
                    }
                });
                
                $("#new_usuario").submit(function(event){
                    event.preventDefault();
                    $("#img_loader").show();
                    $.post(
                        "<?php echo site_url('usuario/recibirdatos'); ?>",
                        {
                            titulo: $("#titulo").val(),
                            login: $("#login").val(),
                            clave: $("#clave").val(),
    ...
    <body>
        ...
        <div class="row text-center color1">
            <h3>INGRESAR UN NUEVO USUARIO</h3>
        </div>
        ...
        <form id="new_usuario" class="form-horizontal form-margin" role="form" method="post" action="">
            <div class="form-group" style="margin-top: 8px">
                <label for="cboPerfil" class="col-sm-2 control-label">Perfil:</label>
                <div class="col-sm-4">
                    <select class="form-control" id="cboPerfil" name="cboPerfil" type="text" placeholder="Login del Usuario" required></select>
                </div>    
            </div>
            <div class="form-group" style="margin-top: 8px">
                <label for="titulo" class="col-sm-2 control-label">Título:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="titulo" name="titulo" type="text" placeholder="Título del Usuario" value="" required autofocus>
                </div>    
            </div>
            <div class="form-group" style="margin-top: 8px">
                <label for="login" class="col-sm-2 control-label">Login:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="login" name="login" type="text" placeholder="Login del Usuario" value="" required>
                </div>    
            </div>
            <div class="form-group" style="margin-top: 8px">
                <label for="clave" class="col-sm-2 control-label">Clave:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="clave" name="clave" type="text" placeholder="Clave del Usuario" value="" required>
                </div>    
            </div>
            <div class="form-group" style="margin-top: 8px">
                <label for="nombre_completo" class="col-sm-2 control-label">Nombre Completo:</label>
                <div class="col-sm-10">
                    <input class="form-control" id="nombre_completo" name="nombre_completo" type="text" placeholder="Nombre completo del Usuario" value="" required>
                </div>    
            </div>
